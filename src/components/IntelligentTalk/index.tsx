  ...
  const getPreparationData = async () => {
    if (uuid) {
      ...
      if (res?.code === 0) {
        ...
        // const baseUrl = 'yantu-playground.anatta.vip:8090' // TODO（配置环境变量）测试
        const baseUrl = 'yantu-ai-b.anatta.vip:8090' // 正式
        connectSocket(
          `wss://${baseUrl}/v1/ws/open/chat/${res.data.chat_uuid}?Authorization=${res.data.token}`
        )
      } else {
